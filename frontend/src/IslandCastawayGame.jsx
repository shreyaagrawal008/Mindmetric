...
const IslandCastawayGame = ({ question, dataStr, onVictory }) => {
    ...
    const handleTap = () => {
        if (isVictory || currentTally >= 5) return;
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex flex-col items-center mt-[-10vh]">
                ...
                <div className={`relative flex items-center justify-center w-64 h-64 md:w-80 md:h-80 transition-all duration-700 ${isVictory ? 'drop-shadow-[0_0_40px_rgba(250,204,21,0.8)] scale-110' : ''}`}>
                ...
                <AnimatePresence>
                    {isVictory && (
                        ...
                        >
                            <span className="text-8xl font-black text-yellow-400 drop-shadow-[0_4px_10px_rgba(0,0,0,0.8)]">5</span>
                        </motion.div>
                    )}
                </AnimatePresence>
            </div>
            ...
            {!isVictory && (
                <div className="absolute bottom-8 z-20 pointer-events-none">
                    <p className="text-white text-xl md:text-2xl font-bold bg-black/50 px-6 py-3 rounded-full animate-pulse border border-yellow-500/50">
                        Tap anywhere to draw a tally! ({currentTally}/5)
                    </p>
                </div>
            )}
