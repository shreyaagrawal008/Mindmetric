...
const RetroRocketGame = ({ dataStr, question, onVictory }) => {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-between touch-none select-none bg-slate-900 font-mono" style={{ background: 'linear-gradient(to bottom, #0f172a 0%, #1e1b4b 100%)' }}>
      ...
      {/* Dashboard UI */}
      <div className="w-full p-2 md:p-4 bg-slate-800 border-t-4 border-slate-600 z-40 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] flex flex-row items-center justify-center gap-4 md:gap-12">
          
          {/* Dashboard Screen (Target Box) */}
          <div className="w-auto max-w-[200px] md:max-w-sm bg-slate-900 border-4 border-slate-700 rounded-xl p-2 md:p-3 shadow-inner relative flex flex-col items-center">
              {/* Status Bar */}
              <div className="w-full flex justify-between items-center mb-2 gap-2">
                  <div className="flex gap-1 items-center">
                      <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full flex-shrink-0 ${isVictory ? 'bg-green-500 shadow-[0_0_10px_#22c55e]' : 'bg-red-500 shadow-[0_0_10px_#ef4444] animate-pulse'}`}></div>
                      <span className="text-[8px] md:text-xs text-slate-400 tracking-widest whitespace-nowrap">{isVictory ? 'SYSTEM ALIGNED' : 'IGNITION LOCKED'}</span>
                  </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-2 md:gap-4 bg-black/50 p-2 md:p-3 rounded-lg border border-slate-800 w-full">
                  ...
                  <div className="relative w-12 h-16 md:w-16 md:h-20 bg-slate-800/80 rounded border-2 border-dashed border-cyan-500/50 flex flex-col items-center justify-center overflow-hidden flex-shrink-0">
                      <span className="text-cyan-500/30 text-[8px] md:text-[10px] absolute top-1 uppercase tracking-widest">Prior</span>
                      <AnimatePresence>
                          {!isVictory && (
                              <motion.div 
                                  className="text-2xl md:text-3xl text-cyan-400 font-bold animate-pulse"
                      ...
                      <AnimatePresence>
                          {isVictory && (
                              ...
                              >
                                  <span className="text-2xl md:text-4xl font-black text-cyan-300 drop-shadow-[0_0_8px_#22d3ee]">{data.target - 1}</span>
                              </motion.div>
                          )}
                      </AnimatePresence>
                  </div>
                  ...
                  {/* Connector */}
                  <div className="flex flex-col gap-1 items-center justify-center opacity-50 flex-shrink-0">
                      <div className="w-2 md:w-3 h-1 bg-cyan-500"></div>
                  </div>

                  {/* The Target Number */}
                  <div className="relative w-12 h-16 md:w-16 md:h-20 bg-slate-800 rounded border-2 border-slate-600 flex flex-col items-center justify-center shadow-[inset_0_0_15px_rgba(0,0,0,0.8)] flex-shrink-0">
                      <span className="text-slate-400 text-[8px] md:text-[10px] absolute top-1 uppercase tracking-widest">Target</span>
                      <span className="text-2xl md:text-4xl font-black text-white drop-shadow-md">{data.target}</span>
          ...
          {/* Keypad Options */}
          <div className="flex flex-wrap md:flex-row justify-center items-center gap-2 md:gap-6 max-w-[200px] md:max-w-none">
              {options.map((opt, index) => {
                  ...
                  return (
                      <motion.button
                          ...
                          className="relative w-14 h-10 md:w-20 md:h-16 bg-slate-700 rounded-lg border-b-4 md:border-b-8 border-slate-900 flex items-center justify-center shadow-lg hover:bg-slate-600 active:border-b-0 active:translate-y-1 md:active:translate-y-2 transition-all group overflow-hidden"
                      ...
                      >
                          ...
                          {/* Button text */}
                          <span className="text-xl md:text-3xl font-black text-slate-200 group-hover:text-cyan-300 drop-shadow-[0_0_5px_rgba(0,0,0,0.5)] group-hover:drop-shadow-[0_0_8px_#22d3ee] transition-all">{opt}</span>
                      </motion.button>
                  );
              })}
          </div>
