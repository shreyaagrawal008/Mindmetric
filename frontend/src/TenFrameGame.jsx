  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center p-4 pt-4 overflow-hidden touch-none">
      ...
      <div className="flex flex-col items-center justify-center w-full mt-2 md:mt-4">
        <div className="grid grid-cols-5 grid-rows-2 gap-2 md:gap-4 p-3 md:p-5 bg-blue-900/60 border-8 border-indigo-400 rounded-2xl shadow-[0_0_40px_rgba(99,102,241,0.5)]">
          {Array.from({ length: 10 }).map((_, index) => {
            ...
            return (
              <motion.div
                ...
                className={`w-12 h-12 md:w-16 md:h-16 border-4 rounded-xl flex items-center justify-center text-2xl md:text-4xl shadow-inner ${showComplement ? 'animate-pulse' : ''}`}
              >
                <AnimatePresence>
                  {isFilled && (
                    <motion.div
      ...
      <div className="w-full max-w-3xl flex flex-row items-center justify-center gap-6 md:gap-12 mt-6 md:mt-8 pb-4">
        {options.map((opt, i) => {
          const isSelected = selectedOption === opt;
          const isCorrect = opt === answerStr;
          
          let cardStyle = "w-20 h-28 md:w-28 md:h-36 rounded-2xl border-4 flex items-center justify-center text-4xl md:text-5xl font-black shadow-xl cursor-pointer transition-all duration-300 bg-gradient-to-br from-indigo-500 to-purple-600 border-indigo-300 text-white hover:scale-105 hover:-translate-y-2 ";
          
          if (isSelected) {
            if (gameState === 'incorrect') {
              cardStyle = "w-20 h-28 md:w-28 md:h-36 rounded-2xl border-4 flex items-center justify-center text-4xl md:text-5xl font-black shadow-xl cursor-pointer transition-all duration-300 bg-red-600 border-red-300 text-white animate-shake ";
            }
          }
          
          if (gameState === 'correct') {
             if (isCorrect) {
                cardStyle = "w-20 h-28 md:w-28 md:h-36 rounded-2xl border-4 flex items-center justify-center text-4xl md:text-5xl font-black shadow-[0_0_40px_rgba(74,222,128,1)] transition-all duration-300 bg-green-500 border-green-200 text-white scale-110 ";
             } else {
                cardStyle = "w-20 h-28 md:w-28 md:h-36 rounded-2xl border-4 flex items-center justify-center text-4xl md:text-5xl font-black shadow-xl transition-all duration-300 bg-slate-700 border-slate-500 text-slate-400 opacity-50 scale-95 pointer-events-none ";
             }
          }
